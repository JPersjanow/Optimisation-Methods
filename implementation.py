...
class LpProblem(object):
    ...
    def addConstraint(self, constraint, name = None):
        ...
        if name in self.constraints:
            if self.noOverlap:
                raise JError("overlapping constraint names: " + name)
            else:
    ...
    def writeLP(self, filename, writeSOS = 1, mip = 1):
        ...
        if long_names:
            raise JError('Variable names too long for Lp format\n'
                            + str(long_names))
        ...
        repeated_names = [(key, value) for key, value in list(repeated_names.items())
                          if value >= 2]
        if repeated_names:
            raise JError('Repeated variable names in Lp format\n'
                            + str(repeated_names))
    ...
    def setSolver(self,solver = LpSolver):
...
class FractionElasticSubProblem(FixedElasticSubProblem):
    ...
    def __init__(self, name, numerator, RHS, sense,
        ...
        if denominator is None and complement is not None:
            ...
            raise JError('only one of denominator and complement must be specified')
        self.RHS = RHS
